    ...
    >
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ───────── MOBILE RESPONSIVE ───────── */
        .grid-3-cols { display: grid; grid-template-columns: repeat(3, 1fr); gap: 22px; }
        .grid-2-cols { display: grid; grid-template-columns: 1fr 1fr; gap: 70px; align-items: center; }
        .grid-2-small { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 28px; }
        .grid-testimonials { display: grid; grid-template-columns: repeat(3, 1fr); gap: 18px; margin-top: 44px; }
        .grid-footer { display: grid; grid-template-columns: 2fr 1fr 1fr 1fr; gap: 40px; margin-bottom: 40px; }
        
        @media (max-width: 1024px) {
          .btn-primary, .btn-outline { padding: 11px 24px; font-size: 10px; }
          .card-wrap { height: auto; }
          .grid-2-cols { gap: 50px; }
          .grid-footer { gap: 30px; }
        }
        
        @media (max-width: 768px) {
          .btn-primary, .btn-outline { padding: 10px 20px; font-size: 10px; }
          .nav-link { font-size: 11px; letter-spacing: 1px; }
          .pill-btn { padding: 8px 16px; font-size: 11px; }
          .grid-3-cols { grid-template-columns: repeat(2, 1fr); gap: 16px; }
          .grid-2-cols { grid-template-columns: 1fr; gap: 30px; }
          .grid-2-small { grid-template-columns: 1fr; gap: 8px; }
          .grid-testimonials { grid-template-columns: repeat(2, 1fr); gap: 14px; }
          .grid-footer { grid-template-columns: 1fr 1fr; }
        }
        
        @media (max-width: 640px) {
          .btn-primary:hover, .btn-outline:hover { transform: translateY(-1px); }
          .nav-link { font-size: 10px; letter-spacing: 0.5px; }
          .pill-btn { padding: 7px 14px; font-size: 10px; }
          .d1 { font-size: 10px !important; }
          .d3 { font-size: 14px !important; }
          .d5 { gap: 16px !important; }
          .grid-3-cols { grid-template-columns: 1fr; gap: 14px; }
          .grid-testimonials { grid-template-columns: 1fr; gap: 12px; }
          .grid-footer { grid-template-columns: 1fr; }
        }
        
        @media (max-width: 480px) {
          .btn-primary, .btn-outline { padding: 9px 18px; font-size: 9px; }
          .nav-link { font-size: 9px; letter-spacing: 0px; }
          .pill-btn { padding: 6px 12px; font-size: 9px; }
